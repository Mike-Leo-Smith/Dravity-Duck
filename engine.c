...
static Return_code engine_loop(Level *level)
{
	...
	while (1)
	{
		...
		if (key_down(K_OPTN))
		{
			if (!optn) display_fps = !display_fps;
			optn = 1;
		}
		else optn = 0;
...
static void engine_movements(Level *level)
{
	...
	if (time_getTicks() - time > 1)
	{
		...
		if (level->gravity == UP || level->gravity == DOWN)
		{
			...
			{
				duck->look = LEFT;
				if (!engine_hitMap(level, duck, -2, 0)) duck->dx = -2;
			}
		...
		else
		{
			...
			{
				duck->look = UP;
				if (!engine_hitMap(level, duck, 0, -2)) duck->dy = -2;
			}
		...
		for (i = SPEED_FALLING; i > 0; i--)
		{
			...
			{
			case UP:
				if (!engine_hitMap(level, duck, 0, -i)) duck->y -= i, i = 0;
				break;
			case DOWN:
				if (!engine_hitMap(level, duck, 0, i)) duck->y += i, i = 0;
				break;
			case LEFT:
				if (!engine_hitMap(level, duck, -i, 0)) duck->x -= i, i = 0;
				break;
			case RIGHT:
				if (!engine_hitMap(level, duck, i, 0)) duck->x += i, i = 0;
				break;
	...
	{ // 4
		...
		for (i = 0; level->object[i]; i++)
		{
			...
			case CATERPILLAR:
				...
				{
				case UP:
					if (engine_hitMap(level, object, object->dx, 0) || !engine_hitMap(level, object, object->dx * TILE_WIDTH, TILE_HEIGHT)) object->dx *= -1, object->look = (object->look + 2) % 4;
					break;
				case DOWN:
					if (engine_hitMap(level, object, object->dx, 0) || !engine_hitMap(level, object, object->dx * TILE_WIDTH, -TILE_HEIGHT)) object->dx *= -1, object->look = (object->look + 2) % 4;
					break;
				case LEFT:
					if (engine_hitMap(level, object, 0, object->dy) || !engine_hitMap(level, object, TILE_WIDTH, object->dy * TILE_HEIGHT)) object->dy *= -1, object->look = (object->look + 2) % 4;
					break;
				case RIGHT:
					if (engine_hitMap(level, object, 0, object->dy) || !engine_hitMap(level, object, -TILE_WIDTH, object->dy * TILE_HEIGHT)) object->dy *= -1, object->look = (object->look + 2) % 4;
					break;
			...
			case HOMING_BULLET:
				if (object->var == 6)
				{
					min_distance = (duck->x - object->x - homing_bullet[object->position][0]) * (duck->x - object->x - homing_bullet[object->position][0]) + (duck->y - object->y - homing_bullet[object->position][1]) * (duck->y - object->y - homing_bullet[object->position][1]);
					...
					distance = (duck->x - object->x - homing_bullet[n][0]) * (duck->x - object->x - homing_bullet[n][0]) + (duck->y - object->y - homing_bullet[n][1]) * (duck->y - object->y - homing_bullet[n][1]);
					if (distance < min_distance) object->position = n;
					object->dx = homing_bullet[object->position][0];
					object->dy = homing_bullet[object->position][1];
					object->var = 0;
				}
				else object->var++;
				object->x += object->dx;
			...
			case PESTLE:
				...
				{
				case UP:
					object->hitbox.y1 = object->hitbox.y2 - pestle_hitbox[object->position];
					break;
				case DOWN:
					object->hitbox.y2 = object->hitbox.y1 + pestle_hitbox[object->position];
					break;
				case LEFT:
					object->hitbox.x1 = object->hitbox.x2 - pestle_hitbox[object->position];
					break;
				case RIGHT:
					object->hitbox.x2 = object->hitbox.x1 + pestle_hitbox[object->position];
					break;
				}
				break;
			case DUCK_BLAST:
				if (object->var > 32)
				{
					level->object = object_del(level->object, object);
					i--;
				}
				break;
			case BULLET_BLAST:
...
static void engine_anim(Level *level)
{
	...
	if (time_getTicks() - time > 8)
	{
		...
		for (i = 0; level->object[i]; i++)
		{
			...
			case DUCK:
				object->position++;
				if (object->position > 8) object->position = 1;
				break;
			case EGG:
				object->var = !object->var;
				if (object->position < 15 || object->var) object->position++;
				if (object->position == 15) object->position = 0;
				break;
			case GRAVITY_SWITCH:
				object->position = (object->position - object->var + 8) % 8;
				break;
			case CATERPILLAR:
				object->position++;
				if (object->position > 5) object->position = 0;
				break;
			case SHOOTING_PLANT:
				if (object->var < 6 && !(object->var & 1)) object->position = (object->position + 1) % 3;
				object->var = (object->var + 1) % TIME_PLANT;
				break;
			case BEE:
				object->position = (object->position + 1) % 4;
				object->var = (object->var + 1) % TIME_BEE;
				break;
			case PESTLE:
				object->var = (object->var + 1) % TIME_PESTLE;
				if (object->var < 6) object->position = (object->position + 1) % 6;
				break;
			case DUCK_BLAST:
				object->var += 4;
				break;
...
static int engine_collisions(Level *level)
{
		...
		if (object_collision(duck, object))
		{
			...
			case GRAVITY_SWITCH:
				if (!change_gravity)
				{
					...
					tmp = duck->hitbox.x1;
					duck->hitbox.x1 = duck->hitbox.y1;
					duck->hitbox.y1 = tmp; //rotate hitbox
					tmp = duck->hitbox.x2;
					duck->hitbox.x2 = duck->hitbox.y2;
					duck->hitbox.y2 = tmp; //rotate hitbox
					duck->x = object->x;
					duck->y = object->y;
					duck->y = object->y;
				}
	...
	for (i = 0; i < 4; i++)
	{
		...
		{
		case SPIKE_UP:
			if ((duck->y + duck->hitbox.y2) % TILE_HEIGHT >= TILE_HEIGHT / 2) return RETRY;
			break;
		case SPIKE_DOWN:
			if ((duck->y + duck->hitbox.y1) % TILE_HEIGHT < TILE_HEIGHT / 2) return RETRY;
			break;
		case SPIKE_LEFT:
			if ((duck->x + duck->hitbox.x2) % TILE_WIDTH >= TILE_WIDTH / 2) return RETRY;
			break;
		case SPIKE_RIGHT:
			if ((duck->x + duck->hitbox.x1) % TILE_WIDTH < TILE_WIDTH / 2) return RETRY;
			break;
		}
	}
	return ret;
